...
template<typename Ktraits, bool kSameType>
struct LoadDout;

template<typename Ktraits>
struct LoadDout<Ktraits, true> {
    static inline __device__ void run(
        typename Ktraits::output_t *source,
        float (&destination)[Ktraits::kNItems],
        typename Ktraits::BlockLoadT::TempStorage &storage,
        typename Ktraits::BlockLoadOutputT::TempStorage &,
        int seqlen
    ) {
        typename Ktraits::input_t loaded[Ktraits::kNItems];
        load_input<Ktraits>(reinterpret_cast<typename Ktraits::input_t *>(source), loaded, storage, seqlen);
        Converter<typename Ktraits::input_t, Ktraits::kNItems>::to_float(loaded, destination);
    }
};

template<typename Ktraits>
struct LoadDout<Ktraits, false> {
    static inline __device__ void run(
        typename Ktraits::output_t *source,
        float (&destination)[Ktraits::kNItems],
        typename Ktraits::BlockLoadT::TempStorage &,
        typename Ktraits::BlockLoadOutputT::TempStorage &storage,
        int seqlen
    ) {
        static_assert(std::is_same<typename Ktraits::output_t, float>::value,
                      "the flexible output type must be float");
        load_output<Ktraits>(source, destination, storage, seqlen);
    }
};
...
void selective_scan_bwd_kernel(SSMParamsBwd params) {
    ...
    for (int chunk = params.n_chunks - 1; chunk >= 0; --chunk) {
        ...
        __syncthreads();
        LoadDout<Ktraits, std::is_same<output_t, input_t>::value>::run(
            dout, dout_vals, smem_load, smem_load1, params.seqlen - chunk * kChunkSize
        );
        u -= kChunkSize;
        // Will reload delta at the same location if kDeltaSoftplus
        if (!kDeltaSoftplus) { delta -= kChunkSize; }
        dout -= kChunkSize;
