...
def selective_scan_reference(u, delta, A, B, C, D=None, delta_bias=None,
    ...
    """Portable inference fallback and ONNX-export implementation."""
    input_dtype = u.dtype
    delta = delta.float()
    ...
    if C.dim() == 3:
        C = C.unsqueeze(1)
    batch, groups, state_dim, length = (int(value) for value in B.shape)
    u = u.view(batch, groups, -1, length).permute(3, 0, 1, 2).float()
    delta = delta.view(batch, groups, -1, length).permute(3, 0, 1, 2)
    A = A.view(groups, -1, state_dim).float()
    B = B.permute(3, 0, 1, 2).float()
    C = C.permute(3, 0, 1, 2).float()
    outputs = []
    previous = u.new_zeros((batch, groups, A.shape[1], state_dim), dtype=torch.float32)
    for start in range(0, length, 4):
        stop = min(start + 4, length)
        delta_chunk, u_chunk = delta[start:stop], u[start:stop]
        B_chunk, C_chunk = B[start:stop], C[start:stop]
        times = delta_chunk.cumsum(dim=0)
        exponentials = torch.einsum("gdn,lbgd->lbgdn", A, times).exp()
        delta_u_B = torch.einsum("lbgd,lbgn->lbgdn", delta_chunk * u_chunk, B_chunk)
        states = exponentials * (delta_u_B / exponentials).cumsum(dim=0)
        states = states + exponentials * previous.unsqueeze(0)
        outputs.append(torch.einsum("lbgn,lbgdn->lbgd", C_chunk, states))
        previous = states[-1]
    output = torch.cat(outputs, dim=0)
    if D is not None:
        output = output + D.view(groups, -1).float() * u
    return output.permute(1, 2, 3, 0).reshape(batch, -1, length).to(input_dtype)
...
class SelectiveScan(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, nrows=1):
        if (selective_scan_cuda is None and selective_scan_cuda_oflex is None) or torch.onnx.is_in_onnx_export():
            return selective_scan_reference(u, delta, A, B, C, D, delta_bias, delta_softplus, nrows)
        ...
        if selective_scan_cuda is not None:
            ...
            ctx.scan_backend = "mamba"
        else:
            out, x, *rest = selective_scan_cuda_oflex.fwd(u, delta, A, B, C, D, delta_bias, delta_softplus, 1, True)
            ctx.scan_backend = "oflex"
...
def cross_selective_scan(
    ...
    if torch.onnx.is_in_onnx_export():
        horizontal = x.flatten(2, 3)
        vertical = x.transpose(2, 3).flatten(2, 3)
        xs = torch.stack((horizontal, vertical, horizontal.flip(-1), vertical.flip(-1)), dim=1)
    else:
    ...
    if torch.onnx.is_in_onnx_export():
        merged = ys.view(B, K, -1, L)
        merged = merged[:, 0:2] + merged[:, 2:4].flip(-1)
        y = merged[:, 0] + merged[:, 1].view(B, -1, W, H).transpose(2, 3).contiguous().view(B, -1, L)
    else:
...
class SS2D(nn.Module):
    ...
    def forward_core(self, x: torch.Tensor, nrows=-1, channel_first=False, return_high_freq=False):
        nrows = 1
        ...
        # 保存处理后的高频特征  
        x_high_processed = x_high.clone() if return_high_freq else None  
        if self.index<3:
            x0 = x_low
        ...
        x_low_processed = x_low.clone() if return_high_freq else None 
        x = torch.cat((x_low, x_high),dim=1)
        if self.ssm_low_rank:
            x = self.out_rank(x)
        if return_high_freq:  
            return x, x_high_processed,x_low_processed
        return x
    
    def forward(self, x: torch.Tensor, return_high_freq=False, **kwargs):  
        x = self.in_proj(x)  
        if self.d_conv > 1:  
            x = self.act(self.conv2d(x))  
        
        result = self.forward_core(x, channel_first=(self.d_conv > 1), return_high_freq=return_high_freq)  
        
        if return_high_freq:  
            x, x_high,x_low = result  
            out = self.dropout(self.out_proj(x))  
            return out, x_high, x_low
        else:  
            x = result  
            out = self.dropout(self.out_proj(x))  
            return out
...
class TViMBlock(nn.Module):
    ...
    def _forward(self, input: torch.Tensor, return_high_freq=False):  
        if self.ssm_branch:  
            if return_high_freq:  
                op_out, x_high,x_low = self.op(input, return_high_freq=True)  
                x = input + self.drop_path(op_out)  # x_high不需要走一下self.drop_path？
            else:  
        ...
        if return_high_freq:  
            return x, x_high,x_low
        return x  

    def forward(self, input: torch.Tensor, return_high_freq=False):
        if self.use_checkpoint:
            if return_high_freq:  
                raise NotImplementedError("return_high_freq not supported with checkpoint")  
            return checkpoint.checkpoint(self._forward, input) 
        else:
            return self._forward(input,return_high_freq=return_high_freq)
