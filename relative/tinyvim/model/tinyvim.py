...
class StageWithHighFreq(nn.Module):  
    """  
    自定义Stage类,支持返回高频特征  
    """  
    def __init__(self, blocks):  
        super().__init__()  
        # 使用数字索引命名,与nn.Sequential保持兼容  
        for idx, block in enumerate(blocks):  
            self.add_module(str(idx), block)  
      
    def forward(self, x, return_high_freq=False):  
        high_freq_list = []  
        low_freq_list = []
        for name, block in self.named_children():  
            if isinstance(block, TViMBlock) and return_high_freq:  
                # TViMBlock需要支持return_high_freq参数  
                x, x_high,x_low = block(x, return_high_freq=True)  
                high_freq_list.append(x_high)  
                low_freq_list.append(x_low)
            else:  
                x = block(x)  
          
        if return_high_freq and len(high_freq_list) > 0:  
            return x, high_freq_list , low_freq_list 
        return x
...
def Stage(dim, index, layers, mlp_ratio=4.,
    ...
    # return blocks
    return StageWithHighFreq(blocks)
...
class TinyViM(nn.Module):
    ...
    def forward_tokens(self, x, return_high_freq=False):  
        outs = []  
        high_feat_outs = []  
        low_feat_outs = []
        
        for idx, block in enumerate(self.network):  
            # block可能是StageWithHighFreq或Embedding层  
            if isinstance(block, StageWithHighFreq) and return_high_freq:  
                result = block(x, return_high_freq=True)  
                if isinstance(result, tuple):  
                    x, high_freq_list,low_freq_list = result  
                    high_feat_outs.append(high_freq_list)  
                    low_feat_outs.append(low_freq_list)
                else:  
                    x = result  
                    # high_feat_outs.append([])  
            else:  
                x = block(x)  
                # if return_high_freq:  
                    # high_feat_outs.append([])  
        ...
        if self.fork_feat:  
            return (outs, high_feat_outs,low_feat_outs) if return_high_freq else outs  
        return (x, high_feat_outs,low_feat_outs) if return_high_freq else x

    def forward(self, x, return_high_freq=False):  
        x = self.patch_embed(x)  
        result = self.forward_tokens(x, return_high_freq=return_high_freq)  
        
        if return_high_freq:  
            x, high_feat,low_feat = result  
        else:  
            x = result  
        
        if self.fork_feat:  
            # 密集预测任务  
            return (x, high_feat,low_feat) if return_high_freq else x  
